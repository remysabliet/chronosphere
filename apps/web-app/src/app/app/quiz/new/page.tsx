...
// The backend has no "unlimited" concept — it always needs a concrete size to
// know how many generation jobs to enqueue, so unlimited mode falls back to a
// fixed target (mirrors the backend's own SECONDS_PER_QUESTION_ESTIMATE-based
// default for time-boxed quizzes).
const UNLIMITED_TARGET_QUESTION_COUNT = 20;

function buildQuizCreateRequest(
  confirmed: ResolvedThema,
  length: QuizLength,
  allowedTypes: QuestionType[]
): QuizCreateRequest {
  const base = { thema: confirmed.thema, question_types: allowedTypes };
  if (length.mode === 'time')
    return { ...base, time_limit_minutes: length.minutes };
  if (length.mode === 'count')
    return { ...base, question_count: length.questions };
  if (length.mode === 'both')
    return {
      ...base,
      question_count: length.questions,
      time_limit_minutes: length.minutes,
    };
  return { ...base, question_count: UNLIMITED_TARGET_QUESTION_COUNT };
}
...
export default function QuizNewPage() {
  ...
  // below). Persists the quiz and enqueues one generation job per
  // concept/Bloom pair via the outbox — the relay/worker generate questions
  // in the background, so this call returns as soon as the row is written.
  ...
  function handleSend() {
    ...
    // Chatting after a quiz was already generated starts a fresh
    // confirm/size/type round (refine() always mints a new extraction_id) —
    // without this, the next round would reuse this round's stale sizing/
    // type answers and silently enqueue a second quiz on reconfirmation.
    if (justConfirmed) {
      setQuizLength(undefined);
  ...
  if (message.kind === 'confirmed') {
    return (
      <AssistantBubble>
        ...
        <p>Your questions are being generated now and will be ready shortly.</p>
        <p className='text-xs text-muted-foreground'>
          Want something else covered? Tell me below — or check on your quiz
          when you&apos;re ready.
        </p>
        <Button asChild size='sm'>
          <Link href={ROUTES.QUIZ_DETAIL(message.quizId)}>Go to your quiz</Link>
        </Button>
      </AssistantBubble>
    );
  }
