...
- **Transport decision: short polling now, SSE later.** Wrap fetch in `useQuizProgress(quizId)` hook (react-query `refetchInterval: 3000`, stops when ready) so the UI never knows the transport. SSE is deferred until the quiz-session page needs real-time push (design then: worker publishes `{quiz_id, ready, expected}` to Redis pub/sub → `GET /v1/quizzes/{id}/events` via sse-starlette → `EventSource` with cookie auth, since EventSource can't send Authorization headers). Event rate here (~1 batch / 10–30s) doesn't justify SSE plumbing yet.
